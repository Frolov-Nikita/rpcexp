...
namespace ModbusBasic.Device
{
    ...
    /// </summary>
    internal class ModbusRtuOverTcpSlaveNetwork : ModbusSlaveNetwork
    {
        private const int TimeWaitResponse = 1000;
        private readonly object _serverLock = new object();

        private readonly ConcurrentDictionary<string, ModbusMasterRtuOverTcpConnection> _masters =
            new ConcurrentDictionary<string, ModbusMasterRtuOverTcpConnection>();

        private TcpListener _server;
#if TIMER
        private Timer _timer;
#endif
        internal ModbusRtuOverTcpSlaveNetwork(TcpListener tcpListener, IModbusFactory modbusFactory)
            : base(new EmptyTransport(modbusFactory), modbusFactory)
        {
            if (tcpListener == null)
            {
        ...
        public override async Task ListenAsync(CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                var masterConnection = new ModbusMasterRtuOverTcpConnection(client, this, ModbusFactory);
                ...
                if (_server != null)
                {
                    lock (_serverLock)
                    {
                        if (_server != null)
                        {
                            ...
                            {
                                ModbusMasterRtuOverTcpConnection connection;
